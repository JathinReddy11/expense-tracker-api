...
async function isCategoryOwnedByUser(user_id, category_id) {
  return await pool.query('SELECT FROM categories WHERE user_id = ($1) AND category_id = ($2)', [
    user_id,
    category_id,
  ]);
}

async function insertExpense(user_id, category_id, amount, description, expense_date) {
  if (!expense_date) {
    return pool.query(
...
async function listExpenses(
  ...
  sortBy ? (query += `ORDER BY e.${sortBy} `) : (query += 'ORDER BY e.expense_date ');
...
async function changeExpense(expense_id, category_id, amount, description, user_id, expense_date) {
...
async function removeExpense(user_id, expense_id) {
  return await pool.query('DELETE FROM expenses WHERE user_id = ($1) AND expense_id = ($2)', [
    user_id,
    expense_id,
  ]);
}
