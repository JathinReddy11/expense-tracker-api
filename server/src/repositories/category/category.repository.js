...
async function addCategory(user_id, name) {
  return await pool.query('INSERT INTO categories(user_id, name) VALUES($1, $2)', [user_id, name]);
}
...
async function listCategories(user_id, page_number = 1, limit = 10) {
  ...
  const total_count = await pool.query('SELECT COUNT(*) FROM categories WHERE user_id = ($1)', [
    user_id,
  ]);
...
async function categoryDelete(user_id, category_id) {
  return await pool.query('DELETE FROM categories WHERE user_id = ($1) AND category_id = ($2)', [
    user_id,
    category_id,
  ]);
}
